  ...
  return (
    <section id="projects" className="py-20 bg-gradient-to-b from-card/50 to-background">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          className="grid lg:grid-cols-2 gap-6 sm:gap-8 max-w-7xl mx-auto"
              ...
              <Card className="group hover-lift transition-all duration-500 bg-card/50 backdrop-blur-sm border-primary/10 overflow-hidden h-full">
                <CardHeader className="p-4 sm:p-6">
                  <div className="flex items-start justify-between mb-3 sm:mb-4">
                    <div className={`inline-flex p-2 sm:p-3 rounded-lg ${getColorClass(project.color)}`}>
                      <div className="w-6 h-6 sm:w-8 sm:h-8">{project.icon}</div>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-8 w-8 sm:h-10 sm:w-10" asChild>
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                    {project.technologies.map((tech) => (
                      <Badge key={tech} variant="secondary" className="text-xs bg-muted/50 text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors duration-300">
                        {tech}
                      </Badge>
                    ))}
                  </div>
                ...
                <CardContent className="p-4 sm:p-6 pt-0">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                    <Button variant="outline" className="flex-1 group-hover:border-primary group-hover:text-primary transition-all duration-300 text-xs sm:text-sm" asChild>
                      <a href={project.demoUrl} target="_blank" rel="noopener noreferrer">
                        <ExternalLink className="w-3 h-3 sm:w-4 sm:h-4 mr-2" />
                        Live Demo
                      </a>
                    </Button>
                    <Button variant="outline" className="flex-1 group-hover:border-neural group-hover:text-neural transition-all duration-300 text-xs sm:text-sm" asChild>
                      <a href={project.githubUrl} target="_blank" rel="noopener noreferrer">
        ...
        >
          <div className="bg-gradient-glow rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto">
            ...
            <Button variant="outline" className="border-primary text-primary hover:bg-primary hover:text-primary-foreground text-sm" asChild>
